    ...
    <UserLayout>
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-white drop-shadow-[0_0_30px_rgba(16,185,129,0.3)]">HS Codes Management</h2>
            <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-stone-300/85 flex items-center">
              <Package className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5 text-emerald-400" />
          ...
          <div className="flex gap-2 sm:gap-3">
            <button
              ...
              className="inline-flex items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-600 px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-semibold text-white shadow-lg shadow-cyan-900/30 hover:shadow-cyan-900/50 hover:from-cyan-600 hover:to-cyan-700 transition-all duration-200 group disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isFetchingFBR ? (
                <>
        ...
        <div className="rounded-xl sm:rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 p-4 sm:p-6 shadow-lg shadow-emerald-900/20">
          ...
          <div className="mt-4 sm:mt-5 flex items-center justify-between border-t-2 border-white/10 pt-4 sm:pt-5">
            <p className="text-xs sm:text-sm text-stone-300/85 flex items-center">
              <Package className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5 text-emerald-400" />
              Showing <span className="font-bold text-white mx-1">{hsCodes.length}</span> of{' '}
              <span className="font-bold text-white ml-1">{totalHSCodes}</span> HS codes
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl sm:rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg shadow-emerald-900/20 overflow-hidden">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 sm:py-16">
              ...
              <p className="mt-3 sm:mt-4 text-xs sm:text-sm text-stone-300/85 font-medium">Loading HS codes...</p>
            </div>
          ) : hsCodes.length === 0 ? (
            <div className="py-12 sm:py-16 text-center">
              <Package className="h-12 w-12 sm:h-16 sm:w-16 mx-auto text-stone-400 mb-3 sm:mb-4" />
              <p className="text-white font-medium text-sm sm:text-base">No HS codes found</p>
              <p className="text-xs sm:text-sm text-stone-400 mt-1">Create your first HS code to get started</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px]">
                ...
                <tbody className="divide-y divide-white/10">
                  {hsCodes.map((hsCode) => (
                    <tr key={hsCode.id} className="hover:bg-white/5 transition-all duration-200 group">
                      <td className="px-4 sm:px-6 py-3 sm:py-4">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            <p className="font-bold text-white text-base sm:text-lg">{hsCode.hsCode}</p>
                            <p className="text-xs sm:text-sm text-stone-300/85 md:hidden truncate max-w-[200px]">{hsCode.description || 'No description'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 py-3 sm:py-4 hidden md:table-cell">
                        <p className="text-sm sm:text-base text-stone-200/85 flex items-center">
                          <FileText className="h-4 w-4 mr-1.5 text-stone-400 flex-shrink-0" />
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-4">
          <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-xl sm:rounded-2xl bg-slate-900/95 backdrop-blur-xl border-2 border-white/10 shadow-2xl">
            ...
            <form onSubmit={handleCreateHSCode} className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-5">
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-stone-200 mb-2">
                  <Hash className="inline h-4 w-4 mr-1.5 text-emerald-400" />
                ...
                />
                <p className="mt-1 text-xs text-stone-400">Format: XXXX.XXXX</p>
              </div>

              <div>
                <label className="block text-xs sm:text-sm font-semibold text-stone-200 mb-2">
                  <FileText className="inline h-4 w-4 mr-1.5 text-emerald-400" />
          ...
          <div className="w-full max-w-md rounded-xl sm:rounded-2xl bg-slate-900/95 backdrop-blur-xl shadow-2xl border-2 border-white/10">
            ...
            <div className="p-4 sm:p-6">
              <p className="text-sm sm:text-base text-stone-200/85 mb-4">
                Are you sure you want to delete HS Code <span className="font-bold text-white">{selectedHSCode.hsCode}</span>?
              </p>
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3">
                ...
                <button
                  ...
                  className="w-full sm:flex-1 rounded-lg sm:rounded-xl bg-gradient-to-r from-red-600 to-red-700 px-4 sm:px-6 py-2 sm:py-2.5 text-sm font-semibold text-white hover:from-red-700 hover:to-red-800 transition-all duration-200 shadow-lg shadow-red-900/40 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {showFBRModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-4">
          <div className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-xl sm:rounded-2xl bg-slate-900/95 backdrop-blur-xl shadow-2xl border-2 border-cyan-400/20">
            {/* Header */}
            ...
            {/* Search Bar */}
            <div className="sticky top-[88px] sm:top-[96px] lg:top-[104px] bg-slate-900/95 backdrop-blur-xl px-4 sm:px-6 lg:px-8 py-4 border-b-2 border-white/10">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-white/5 backdrop-blur-sm border-2 border-white/20 pl-12 pr-4 py-3 text-sm font-medium text-white placeholder-stone-400 focus:border-cyan-400/50 focus:outline-none focus:ring-4 focus:ring-cyan-500/20 transition-all duration-200"
                  autoFocus
                />
                {fbrSearch && (
                  <button
                    onClick={() => setFbrSearch('')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg bg-white/10 backdrop-blur-sm p-1.5 hover:bg-white/20 transition-colors duration-200"
                  >
              ...
              {selectedFBRCodes.size > 0 && (
                <div className="mt-3 flex items-center justify-between rounded-xl bg-cyan-500/20 backdrop-blur-sm border-2 border-cyan-400/30 px-4 py-2.5">
                  ...
                  <button
                    ...
                    className="text-xs font-semibold text-cyan-300 hover:text-cyan-100 underline transition-colors duration-200"
                  >
                    Clear selection
                  </button>
                </div>
            ...
            <div className="overflow-y-auto p-4 sm:p-6 lg:p-8" style={{ maxHeight: 'calc(92vh - 350px)' }} id="fbr-codes-container">
              {isFilteringFBR ? (
                <div className="py-16 text-center">
                  <div className="h-12 w-12 mx-auto animate-spin rounded-full border-4 border-cyan-500 border-t-transparent shadow-lg mb-4"></div>
                  <p className="text-stone-200/85 font-medium text-base">Filtering codes...</p>
                </div>
              ) : filteredFBRCodes.length === 0 ? (
                <div className="py-16 text-center">
                  <AlertCircle className="h-16 w-16 mx-auto text-stone-400 mb-4" />
                  <p className="text-stone-200/85 font-medium text-base">No HS codes found</p>
                ...
                <>
                  <div className="mb-4 text-sm text-stone-300/85 font-medium">
                      ...
                      return (
                        <button
                          ...
                          className={`group relative rounded-xl border-2 p-4 sm:p-5 text-left transition-all duration-200 ${
                            isSelected
                              ? 'border-cyan-400/50 bg-gradient-to-br from-cyan-500/20 to-cyan-600/10 backdrop-blur-sm shadow-lg shadow-cyan-500/20'
                              : 'border-white/10 bg-white/5 backdrop-blur-sm hover:border-cyan-400/30 hover:bg-white/10 hover:shadow-md'
                          }`}
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className={`flex-shrink-0 h-6 w-6 rounded-lg border-2 flex items-center justify-center transition-all duration-200 ${
                              isSelected
                                ...
                                : 'border-white/30 bg-white/5 backdrop-blur-sm group-hover:border-cyan-400/50'
                            }`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <p className="font-bold text-white text-base sm:text-lg">{code.hS_CODE}</p>
                            </div>
                            <p className="text-sm text-stone-200/85 leading-relaxed">{code.description}</p>
                          </div>
                        </div>

                        {/* Selected Badge */}
                        {isSelected && (
                          <div className="absolute top-3 right-3 rounded-full bg-cyan-500 px-3 py-1 text-xs font-bold text-white shadow-lg">
                            Selected
                          </div>
                        )}
                      </button>
                  ...
                  {displayedFBRCodes.length < filteredFBRCodes.length && (
                    <div className="mt-6 text-center">
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-sm border-2 border-white/20 px-6 py-3 text-sm font-semibold text-white hover:bg-white/20 hover:border-white/30 transition-all duration-200 shadow-lg"
                      >
            ...
            {/* Footer Actions */}
            <div className="sticky bottom-0 bg-slate-900/95 backdrop-blur-xl border-t-2 border-white/10 px-4 sm:px-6 lg:px-8 py-4 sm:py-5 rounded-b-xl sm:rounded-b-2xl">
              <div className="flex flex-col-reverse sm:flex-row gap-3">
                <button
                  ...
                  className="w-full sm:flex-1 rounded-xl border-2 border-white/20 bg-white/5 backdrop-blur-sm px-6 py-3 text-sm font-semibold text-white hover:bg-white/10 hover:border-white/30 transition-all duration-200 shadow-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="w-full sm:flex-1 rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-600 px-6 py-3 text-sm font-semibold text-white hover:from-cyan-600 hover:to-cyan-700 transition-all duration-200 shadow-lg shadow-cyan-900/30 hover:shadow-cyan-900/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
