...
export const InvoicePDF: React.FC<InvoicePDFProps> = ({ invoice, qrCodeDataURL }) => {
  // Calculate totals
  ...
  const getValidationStatus = () => {
    if (!invoice.fbrResponse?.validationResponse) return null;
    const status = invoice.fbrResponse.validationResponse.status?.toLowerCase();
    return status === 'valid' ? 'VALID' : status === 'invalid' ? 'INVALID' : null;
  };
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.footer}>
          <View style={styles.footerLeft}>
            ...
            <Text style={styles.websiteText}>To make FBR Compliant invoices for your business visit: saadtrader.pk</Text>
